...
function removeLastOccurrences(str, value) {
  ...
  if (lastIndex !== -1) {
    ...
    const afterLastOccurrence = str.substring(lastIndex + value.length);
...
function formatTime(minutes, seconds) {
  if (
    ...
    throw new Error(
      'Invalid input. Both minutes and seconds must be non-negative integers.'
    );
  }
...
function getCardId(value) {
  ...
  const rank = ranks.indexOf(value.slice(0, -1));
  const suit = suits.indexOf(value.slice(-1));
